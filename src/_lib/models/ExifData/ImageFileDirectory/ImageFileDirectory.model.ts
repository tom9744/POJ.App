...
export class ImageFileDirectory implements IImageFileDirectory {
  ...
  protected _firstEntryOffset: number;
  ...
  constructor(dataView: DataView, firstIFDOffset: number, isLittle: boolean) {
    this._baseOffset = firstIFDOffset + 10; // NOTE: Byte Align 헤더의 Offset(= 10)을 기준으로 합니다.
    this._firstEntryOffset = this._baseOffset + 2; // NOTE: 실제 IFD 엔트리는 Entry Count 데이터(= 2 Bytes) 이후에 위치합니다.
    ...
    for (let n = 0; n < entryCount; n++) {
      const entry = new IFDEntry(
        dataView,
        this._firstEntryOffset + 12 * n, // NOTE: IFD Entry는 12 Bytes로 구성됩니다.
        isLittle
      );
